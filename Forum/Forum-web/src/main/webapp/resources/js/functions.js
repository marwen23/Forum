...
(function($){
	...
	IGNITE.header = {
		...
		menufunctions: function(){
			...
			if( $body.hasClass('device-lg') || $body.hasClass('device-md') ) {
				$('#primary-menu.sub-title > ul > li,#primary-menu.sub-title > div > ul > li').on('hover', function() {
			...
			if( IGNITE.isMobile.Windows() ) {
				...
				$( '#primary-menu ul li.sub-menu' ).children('a.wn-submenu-trigger').on('click', function(e){
		...
		sidePanel: function(){
			if( $body.hasClass('side-push-panel') ) {
				$("#side-panel-trigger,#side-panel-trigger-close").on('click', function(){
		...
		onePageScroll: function(){
			if( $onePageMenuEl.length > 0 ){
				...
				$onePageMenuEl.find('a[data-href]').on('click', function(){
					var element = $(this),
		...
		topsocial: function(){
			if( $topSocialEl.length > 0 ){
				if( $body.hasClass('device-md') || $body.hasClass('device-lg') ) {
					...
					$topSocialEl.find('a').on('hover', function() {
	...
	IGNITE.portfolio = {
		...
		ajaxload: function(){
			$('.portfolio-ajax .portfolio-item a.center-icon').on('click', function(e) {
		...
		initializeAjax: function( portPostId ){
			...
			$('#next-portfolio, #prev-portfolio').on('click', function() {
	...
	IGNITE.widget = {
		...
		toggles: function(){
			...
			if( $toggle.length > 0 ) {
				$toggle.each( function(){
					...
					element.find('.togglet').on('click', function(){
		...
		accordions: function(){
			...
			if( $accordionEl.length > 0 ){
				$accordionEl.each( function(){
					...
					element.find('.acctitle').on('click', function(){
		...
		navTree: function(){
			...
			if( $navTreeEl.length > 0 ){
				$navTreeEl.each( function(){
					...
					element.find( 'ul li:has(ul) > a' ).on('click', function(e){
		...
		extras: function(){
			...
			$('#primary-menu-trigger,#overlay-menu-close').on('click', function() {
				$( '#primary-menu > ul, #primary-menu > div > ul' ).toggleClass("show");
				return false;
			});
			$('#page-submenu-trigger').on('click', function() {
				$body.toggleClass('top-search-open', false);
				$pagemenu.toggleClass("pagemenu-active");
				return false;
			});
			$pagemenu.find('nav').on('click', function(e){
				$body.toggleClass('top-search-open', false);
				$topCart.toggleClass('top-cart-open', false);
			});
			if( IGNITE.isMobile.any() ){
	...
	$(document).on('ready', IGNITE.documentOnReady.init );
